...
เมื่อตั้งค่าครบ ระบบจะ**สำรองทุกรูปขึ้น Google Drive** และยังเก็บใน `uploads/` บนเครื่องด้วย  
โฟลเดอร์ราก = `GOOGLE_DRIVE_FOLDER_ID` แล้วสร้างโฟลเดอร์ย่อยอัตโนมัติ:

| โฟลเดอร์ย่อย | ใช้กับ |
|-------------|--------|
| `avatars` | รูปโปรไฟล์ |
| `slips` | สลิปโอนเงิน |
| `community` | รูปโพสต์คอมมูนิตี้ |
| `hero` | แบนเนอร์หน้าแรก / แกลเลอรี / สไลด์ |
| `cert` | โลโก้–ตราประทับใบเซอร์ |

แคช ID โฟลเดอร์ย่อยอยู่ที่ `backend/google.drive.folders.json` (ไม่ commit)  
ถ้ายังไม่ตั้งค่า Drive ระบบจะเก็บเฉพาะใน `uploads/` เหมือนเดิม
...
ตอบ: ขึ้นแล้ว — เก็บในโฟลเดอร์ย่อย `hero` ใต้โฟลเดอร์ราก และยังมีไฟล์ใน `uploads/hero/` ด้วย
